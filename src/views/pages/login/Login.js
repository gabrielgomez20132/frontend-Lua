...
const baseUrl = 'https://localhost:44344/api/usuarios_app/existe/persona'
...
export default class login extends Component {
  ...
  validarDni = async () => {
    await axios
      .get(baseUrl + `/${this.state.form.documento}`)
      .then((response) => {
        return response
      })
      .then((response) => {
        ...
        if (response.status == '200') {
          ...
          alert('Hola , ' + response.data.nombres + response.data.apellido)
            ...
            <CCol md="8">
              <CCardGroup>
                <CCard className="p-4">
                  <CCardBody>
                    <CForm>
                      <h1>Para empezar Ingresá tu DNI</h1>
                      <p className="text-medium-emphasis">Sin Puntos</p>
